...
#derive_induction Trie.upsert.loop

theorem valid_loop_upsert {α} (t : Trie α) (i : Nat) (k : String) (f : Option α → α) (ht : Trie.valid t):
    Trie.valid (Trie.upsert.loop k f i t) := by
  induction i, t using Trie.upsert.loop.induct (s := k) (f := f)
  case case1 i v hi =>
    simp only [Trie.upsert.loop, hi, if_pos]; simp_utf8
    apply valid_mkPath; apply Trie.valid.leaf
  case case2 i v hi =>
    simp only [Trie.upsert.loop, hi, if_neg]; simp_utf8
    dsimp -- TODO: how to cleanly resolve if
    apply Trie.valid.leaf
  case case3 i v ps hps t' hi j hj =>
    intro ht -- TODO: Why do we have to intro this here?
    cases ht; case path _ ht =>
    simp only [Trie.upsert.loop, hi, hj, if_pos, if_neg]; simp_utf8
    dsimp
    apply valid_mkPath
    apply IH1
    apply valid_mkPath
  ...
  case case6 i v cs ts hi IH =>
    cases ht; case node hnodup hts hsize =>
    simp only [Trie.upsert.loop, hi, if_pos, if_neg]; simp_utf8; dsimp
    ...
    case h_1 _ hfindnone =>
      apply Trie.valid.node
      · simpa
      · simp only [ByteArray.toList_push, List.nodup_snoc, hnodup, and_true]
    ...
    case h_2 _ idx _hfindsome =>
      ...
      · simp only [← Array.mem_data, List.mem_iff_get?, forall_exists_index]
        ...
        case inl hj =>
          ...
          intro t'' ht'' ht''
          cases ht''
          apply IH -- induction happens here
          apply hts
  ...
  case case7 i v cs ts hi =>
    cases ht; case node hnodup hts hsize =>
    simp only [Trie.upsert.loop, hi, if_pos, if_neg]; simp_utf8; dsimp
    apply Trie.valid.node <;> assumption
